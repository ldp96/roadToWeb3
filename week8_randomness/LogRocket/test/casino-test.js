...
const hashB = ethers.utils.keccak256(valBwin)
...
describe("Casino", async () => {
  ...
  it("Work all the way through (B wins)", async () => {
    ...
    tx = await cB.acceptBet(hashA, hashB, {value: 1e10})
    rcpt = await tx.wait()
    expect(rcpt.events[0].event).to.equal("BetAccepted")      
    ...
    tx = await cB.revealB(valBwin, hashA)
    rcpt = await tx.wait()
    expect(rcpt.events[0].event).to.equal("BRevealed")      
  ...
  it("Work all the way through (A wins)", async () => {
    ...
    tx = await cB.revealB(valBlose, hashA)
    rcpt = await tx.wait()
    expect(rcpt.events[0].event).to.equal("BRevealed")      
